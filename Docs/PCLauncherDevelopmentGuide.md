...
### 背景图被裁得比设计多（2026-09-22）

症状：主界面背景被裁掉太多 —— 用户截图量出来图宽少了 **23%**，而 `object-fit: cover` 的几何下限
只有 9%（1920×983 的图放进 16:9 窗口，左右各 4.5%、纵向不裁），而且裁切明显偏右、不对称。
界面本身看不出异常，只有背景不对。

根因：背景 / 遮罩 / 扫描线原来挂在 `.launcher-shell` 上（`position: absolute`），而外壳为了保住
1200×675 的设计坐标带 `min-width` / `min-height`。**窗口的 CSS 视口比设计尺寸小的时侯**（用户机器上
量到约 1012×571，只有 84%；显示缩放、WebView 缩放都会造成），外壳仍是 1200×675、由 `overflow: hidden`
把右边约 188px、下边约 104px 裁掉 —— 背景正好铺在这块 1200×675 上，就被连带多裁。
界面之所以看着没事：右侧控件要么是 `position: fixed`，要么锚在 `right: 236`（x=964）这类偏左的位置，
刚好还落在可见区内。**所以这条故障只会以"背景不对劲"的形式出现。**

修法：这三层改成 `position: fixed`（锚在视口上），`cover` 直接在"用户看得见的窗口"上取值，
裁切量恒为几何最小值且与窗口/缩放无关；顺带去掉 `transform: scale(1.005)`，改用 1px 外扩
（`left/top: -1px` + `calc(100% + 2px)`）防亚像素缝。平台页 `.platform-game-overview` 本来就是
`position: fixed`，这次只是把外壳对齐它。

守卫：`tests/launcherUiSizeRule.test.ts` →「背景 / 遮罩 / 扫描线锚在窗口上，不跟着画布一起被裁」。

**排错用得上的一招**：拿用户截图里任意控件的右缘像素数 ÷ 它的 CSS 右缘，就反推出窗口的实际 CSS 视口宽度。
